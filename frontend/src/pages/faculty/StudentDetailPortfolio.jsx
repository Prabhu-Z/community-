...
import { ArrowLeft, User, Mail, Phone, BookOpen, Award, CheckCircle2, FileCheck, Printer, CheckSquare, Sparkles, Github, Linkedin, Globe, Link2 } from 'lucide-react';
...
const StudentDetailPortfolio = () => {
  ...
  const fetchStudentDetail = async () => {
    try {
      ...
      const [commRes, actRes, achRes, certRes, taskRes, lbRes] = await Promise.all([
        api.get(`/students/${id}/communities`).catch(() => ({ data: [] })),
        api.get(`/students/${id}/activities`).catch(() => ({ data: [] })),
        api.get(`/students/${id}/achievements`).catch(() => ({ data: [] })),
        api.get(`/students/${id}/certificates`).catch(() => ({ data: [] })),
        api.get(`/tasks/student/${id}`).catch(() => ({ data: [] })),
        api.get('/leaderboard/all').catch(() => ({ data: [] })),
      ]);

      const myLbEntry = (lbRes.data || []).find(e => e.studentId === Number(id));
      if (s) {
  ...
  const activeStudent = student || {
    id: id,
    name: 'Student User',
    studentCode: 'STU' + (10000 + parseInt(id)),
  ...
  return (
    <div className="space-y-8 p-2 lg:p-4">
      ...
      <button
        onClick={() => navigate(-1)}
        className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs flex items-center gap-2 shadow-sm transition self-start"
      >
        <ArrowLeft className="w-4 h-4 text-[#8b5cf6]" /> Back to Student Search
      </button>
      ...
      <div className="bg-white p-6 lg:p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <h1 className="text-3xl font-extrabold text-slate-900 mt-0.5">{activeStudent.name}</h1>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-3">
            ...
            <div className="space-y-2.5 text-xs">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-slate-800 font-bold">{activeStudent.email || 'N/A'}</span>
              </div>
              <div className="flex items-center justify-between">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
          ...
          {communities.length > 0 ? (
            ...
            <div className="p-8 text-center text-xs text-slate-500 font-medium">
              No joined communities found yet.
            </div>
          )}
        </div>
